...
import EmailDogrulama from "./pages/EmailDogrulama";
...
function App() {
  ...
  useEffect(() => {
    ...
    // Check and create admin column if it doesn't exist
    const checkAndCreateAdminColumn = async () => {
      try {
        // First check if any user has the admin field
        const { data: existingData, error: queryError } = await supabase
          .from('kullanicilar')
          .select('admin')
          .limit(1);
          
        if (queryError && queryError.message && queryError.message.includes('admin')) {
          console.log('Admin column may not exist, proceeding with migration plan');
          
          // Attempt to get the first user to check if we have data
          ...
          if (users && users.length > 0) {
            // We have users, update them to add admin field
            const { error: updateError } = await supabase.rpc('add_admin_column_to_kullanicilar');
            
            if (updateError) {
              console.error('Error updating users:', updateError);
              
              // Fallback: try to update each user individually
            ...
            console.log('No users exist yet, admin column will be created on first insert');
          }
        } else {
          console.log('Admin column exists');
        }
      } catch (error) {
        console.error('Error checking admin column:', error);
      }
    };
    
    checkAndCreateAdminColumn();
  }, []);
              ...
              <>
                ...
                <main>
                  <Routes>
                    ...
                    <Route path="/email-dogrulama" element={<EmailDogrulama />} />
                  </Routes>
                </main>
